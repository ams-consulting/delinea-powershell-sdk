...
function Delinea.Platform.PowerShell.Redrock.CreateQuery {
	param (
	...
	try	{
		...
		$RedrockQuery.Header 		= @{ "X-CENTRIFY-NATIVE-CLIENT" = "true"; }
...
function Delinea.Platform.PowerShell.Redrock.GetQueryFromFile {
	param (
...
function Delinea.Platform.PowerShell.Redrock.GetIDFromPrincipal {
	param (
	...
	try {
		...
		# Build Query
		$RedrockQuery = Delinea.Platform.PowerShell.Redrock.CreateQuery -Query $Query
